...
class Marks:
    ...
    def add_mark(self, student_id, course_id, mark):
        ...
        self.__marks[student_id][course_id] = math.floor(mark * 10) /10
...
class Utils:
    ...
    def calculate_gpa(self, student_id):
        student_marks = self.marks.get_all_marks().get(student_id)
        ...
        for course_id, mark in student_marks.items():
            ...
            if course:
                course_weight = float(input(f"Enter weight for course {course_id}: "))  # Fixed line
                course_weights.append(course_weight)
                course_grades.append(mark)

        if not course_weights or not course_grades:
            return None

        weights = np.array(course_weights)
        grades = np.array(course_grades)

        gpa = np.average(grades, weights=weights)
    ...
    def display_student_information(self):
        ...
        for student in self.students:
            gpa = self.calculate_gpa(student.get_student_id())
            if gpa is not None:
        ...
        for student, gpa in student_gpas:
            print(
                f"Student ID: {student.get_student_id()}, Name: {student.get_name()}, DOB: {student.get_dob()}, GPA: {gpa:.1f}"
            )
...
def main():
    ...
    while True:
        # Print the menu
        screen.addstr("Menu:\n")
        screen.addstr("0. Exit\n")
        screen.addstr("1. Input number of students\n")
        screen.addstr("2. Input student information\n")
        screen.addstr("3. Input number of courses\n")
        screen.addstr("4. Input course information\n")
        screen.addstr("5. Input marks\n")
        screen.addstr("6. Display student information\n")
        screen.addstr("7. Display course information\n")
        screen.addstr("8. Display marks\n")

        # Get user input
        choice = screen.getch() - ord('0')
    ...
    # End curses and return to the normal terminal
    curses.endwin()
